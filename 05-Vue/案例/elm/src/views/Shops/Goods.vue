  ...
  <div class="goods" v-if="shopInfo">
    ...
    <div class="menuview">
      ...
      <div class="menu-wrapper" ref="menuScroll">
        <ul>
          <!-- 遍历得到十二项 -->
          <li v-for="(item,index) in shopInfo.menu" :key="index" @click="selectMenu(index)">
...
export default {
  ...
  computed: {
    ...
    currentIndex() {
      for (let i = 0; i < this.listHeight.length; i++) {
        // 遍历所有列表的高度，以便得出一块区域的高度
        let height1 = this.listHeight[i];
        let height2 = this.listHeight[i + 1];
        // 判断对应滑动的商品位于这块范围内，那么返回索引
  ...
  methods: {
    ...
    calculateHeight() {
      ...
      let height = 0; // 第一片数据区域的高度
      this.listHeight.push(height);
      for (let i = 0; i < foodlist.length - 1; i++) {
        let item = foodlist[i];
        // item.clientHeight得到每一片数据的高度
