...
class _ProductListState extends State<ProductList> {
  ...
  void initState() {
    super.initState();
    this._cid = widget.arguments["cid"];//在widget中早已定义好了参数 
    this._keywords = widget.arguments["keywords"];
    // 给search框赋值
    this._initKeywordsController.text = this._keywords;//将关键字赋给了搜索框
    _getProductListData();
  ...
  _getProductListData() async {
    ...
    // 根据历史记录有无来更改请求数据的api
    var api;
    if(this._keywords==null){//此时不存在历史记录,利用的是cid
      api =
    ...
    if(productList.result.length==0 && this._page==1){
      setState(() {
       this._hasData = false;//此时已不存在数据 
      });
    }else{
      this._hasData = true;//仍有数据
    }
    // 判断最后一页有没有数据
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: _hasData ? Stack(//判断是否存在搜索的数据，存在就渲染，不存在文本渲染
